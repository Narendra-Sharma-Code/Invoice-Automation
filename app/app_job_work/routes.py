   ...
   # Form input
    ...
    try:
        ...
        if return_switch == "on":
            ...
            # --- Additional Code Start: Fuzzy Matching Setup & Debugging ---
            ...
            # Build a header mapping from the header row (assumed to be 'start_row')
            column_mapping = {}
            for col in range(1, ws.max_column + 1):
                column_val = ws.cell(row=start_row, column=col).value
                if column_val:
                    # Convert to string to avoid attribute errors (e.g., for floats)
                    column_mapping[col] = str(column_val).strip()
            print("DEBUG: column_mapping =", column_mapping)
            ...
            for balance_row in balance_table[:-1]:
                ...
                if all(key in balance_row for key in ['balance_met_wt_gms', 'balance_value_usd', 'rm']):
                    ...
                    # --- Additional Code Start: Fuzzy Matching & Debug Logging ---
                    # Make sure to treat both user_rm and balance_row['rm'] as strings
                    row_rm = str(balance_row['rm'])
                    for user_rm in rm_list:
                        user_rm_str = str(user_rm)
                        # Debug: Print similarity between user input and the row's RM
                        sim_ratio_rm = difflib.SequenceMatcher(None, user_rm_str.lower(), row_rm.lower()).ratio()
                        print(f"DEBUG: Similarity between user_rm '{user_rm_str}' and row RM '{row_rm}' is {sim_ratio_rm:.2f}")
                        if is_similar(user_rm_str, row_rm, threshold=0.6):
                            # If a match is found, look for a matching header.
                            for col_idx, header_text in column_mapping.items():
                                header_text_str = str(header_text)
                                sim_ratio_header = difflib.SequenceMatcher(None, user_rm_str.lower(), header_text_str.lower()).ratio()
                                print(f"DEBUG: Similarity between user_rm '{user_rm_str}' and header '{header_text_str}' is {sim_ratio_header:.2f}")
                                if is_similar(user_rm_str, header_text_str, threshold=0.6):
                                    col_letter = get_column_letter(col_idx)
                                    print(f"  -> For user_rm '{user_rm_str}' matching header '{header_text_str}', writing gross wt {abs(balance_row['balance_met_wt_gms'])} at {col_letter}{i}")
                                    ws[f'{col_letter}{i}'] = abs(balance_row['balance_met_wt_gms'])
                                    # Once a matching header is found, break out of the header loop.
                                    break
                    # --- Additional Code End ---
